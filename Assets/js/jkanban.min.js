...
(function () {
  this.jKanban = function () {
    ...
    var __DEFAULT_ITEM_REMOVE_OPTIONS = {
      enabled: false
    }
    ...
    var defaults = {
      ...
      propagationHandlers: [],
      itemRemoveOptions: __DEFAULT_ITEM_REMOVE_OPTIONS
    }
    ...
    this.addBoards = function (boards, isInit) {
      ...
      var rmButton = self.options.itemRemoveOptions.enabled
      ...
      for (var boardkey in boards) {
        ...
        boardNode.dataset.boardkey = boardkey
        ...
        headerBoard.innerHTML =
          '<div class="kanban-title-board" contentEditable="true">' + board.title + '</div>'
        
        var navBoard = document.createElement('nav');
        navBoard.classList.add('kanban-board-nav')
        if(board.total_pages > 1) {
          
          var navHtml = '<button class="kanban-board-nav-prev btn btn-default btn-xs pull-left '+(board.current_page > 1 ? '' : 'disabled')+'" data-boardKey="'+boardkey+'"><span class="fa fa-arrow-left"></span></button>'
          
          
          navHtml += '<button class="kanban-board-nav-next btn btn-default btn-xs pull-right '+(board.current_page < board.total_pages ? '' : 'disabled')+'" data-boardKey="'+boardkey+'"><span class="fa fa-arrow-right"></span></button>'
          navHtml += '<div class="text-center"><span class="current_page">'+board.current_page+'</span>/'+board.total_pages+'</div>';
          
          navBoard.innerHTML = navHtml
        }
        ...
        if(rmButton){
          ...
          btn_rm.setAttribute('class', 'kanban-title-button btn btn-default btn-xs kanban-remove-board');
          btn_rm.innerHTML = '<i class="fa fa-times"></i>';
          __onButtonRemoveClickHandler(btn_rm, board.id);
          headerBoard.appendChild(btn_rm);
        }
        var totals = document.createElement('div');
        totals.innerHTML = '<div class="kanban-totals"><span class="pull-left total_value">'+ mauticLang['plugin.powerticpipes.currency_code']+'<span>'+number_format(board.total_value, 2, ',', '.')+'</span></span> <span class="pull-right total_items"><span>' +number_format(board.total_items, 0, '', '.')+ '</span> '+mauticLang['plugin.powerticpipes.prospects']+'</span></div>';
        headerBoard.appendChild(totals)
        //board assembly
        boardNode.appendChild(headerBoard)
        boardNode.appendChild(navBoard)
    ...
    this.buttonRemoveClick = function (el) {}
    ...
    function __onButtonRemoveClickHandler (nodeItem, boardId) {
      nodeItem.addEventListener('click', function (e) {
        e.preventDefault()
        self.options.buttonRemoveClick(this, boardId)
      })
    }
      ...
      for (var propertyName in parentObject) {
        ...
        if(typeof parentObject[propertyName] === 'object') {
          element.setAttribute(
            'data-' + propertyName,
            JSON.stringify(parentObject[propertyName])
          )
        } else {
    ...
    function __buildItemCard(item) {
      ...
      var result = '';
      if(self.options.itemRemoveOptions.enabled){
        result += '<div class="kanban-item-remove pull-right btn btn-default btn-xs"><i class="fa fa-times"></i></div>';
      }

      result += 'title' in item ? '<div class="kanban-item-title">'+item.title+'</div>' : '<div class="kanban-item-title">'+mauticLang['plugin.powerticpipes.empty_card']+'</div>';
      result += '<hr class="clearfix" />';
      if(item.creator.trim() != ""){
        result += '<div class="kanban-item-creator  small"><b>'+mauticLang['plugin.powerticpipes.createdBy']+':</b> <span>'+item.creator+'</span></div>';
      }
      
      if(typeof item.lead != 'undefined'){
        if(cardColumns.indexOf('lead.name') > -1 && item.lead.name != '' && item.lead.name != ' ' && item.lead.name != null){
          result += '<a class="kanban-item-lead small kanban-item-lead-contact" title="'+item.lead.name+'" href="/s/contacts/view/'+item.lead.id+'"><b>'+mauticLang['plugin.powerticpipes.contact']+':</b> <span>'+item.lead.name+'</span></a>';
        }
        if(cardColumns.indexOf('lead.email') > -1 && item.lead.email != '' && item.lead.email != ' ' && item.lead.email != null){
          result += '<div class="kanban-item-lead small" title="'+item.lead.email+'"><b>'+mauticLang['plugin.powerticpipes.email']+':</b> <span>'+item.lead.email+'</span></div>';
        }
        if(cardColumns.indexOf('lead.company') > -1 && item.lead.company != '' && item.lead.company != ' ' && item.lead.company != null){
          result += '<div class="kanban-item-lead small" title="'+item.lead.company+'"><b>'+mauticLang['plugin.powerticpipes.company']+':</b> <span>'+item.lead.company+'</span></div>';
        }

        if(cardColumns.indexOf('lead.points') > -1 && item.lead.points != '' && item.lead.points != ' ' && item.lead.points != null){
          result += '<div class="kanban-item-lead  small"><b>'+mauticLang['plugin.powerticpipes.points']+':</b> <span>'+item.lead.points+'</span></div>';
        }

        if(cardColumns.indexOf('lead.position') > -1 && item.lead.position != '' && item.lead.position != ' ' && item.lead.position != null){
          result += '<div class="kanban-item-lead  small"><b>'+mauticLang['plugin.powerticpipes.position']+':</b> <span>'+item.lead.position+'</span></div>';
        }

        if(cardColumns.indexOf('lead.phone') > -1 && item.lead.phone != '' && item.lead.phone != ' ' && item.lead.phone != null){
          result += '<div class="kanban-item-lead  small"><b>'+mauticLang['plugin.powerticpipes.phone']+':</b> <span>'+item.lead.phone+'</span></div>';
        }

        if(cardColumns.indexOf('lead.mobile') > -1 && item.lead.mobile != '' && item.lead.mobile != ' ' && item.lead.mobile != null){
          result += '<div class="kanban-item-lead  small"><b>'+mauticLang['plugin.powerticpipes.mobile']+':</b> <span>'+item.lead.mobile+'</span></div>';
        }

        if(cardColumns.indexOf('lead.address') > -1 && item.lead.address != '' && item.lead.address != ' ' && item.lead.address != null){
          result += '<div class="kanban-item-lead small" title="'+item.lead.address+'"><b>'+mauticLang['plugin.powerticpipes.address']+':</b> <span>'+item.lead.address+'</span></div>';
        }

        if(cardColumns.indexOf('lead.country') > -1 && item.lead.country != '' && item.lead.country != ' ' && item.lead.country != null){
          result += '<div class="kanban-item-lead  small"><b>'+mauticLang['plugin.powerticpipes.country']+':</b> <span>'+item.lead.country+'</span></div>';
        }
        if(cardColumns.indexOf('lead.date_modified') > -1){
          result += '<div class="kanban-item-lead  small"><b>'+mauticLang['plugin.powerticpipes.date_modified']+':</b> <span>'+item.lead.date_modified+'</span></div>';
        }
        
      }


      if(cardColumns.indexOf('powertic_pipes_cards.date_added') > -1){
        result += '<div class="kanban-item-date_added  small"><b>'+mauticLang['plugin.powerticpipes.card.date_added']+':</b> '+item.date_added+'</div>';
      }

      
      if(cardColumns.indexOf('powertic_pipes_cards.value') > -1 && item.value != '' && item.value != null){
        result += '<div class="kanban-item-value  small"><b>'+mauticLang['plugin.powerticpipes.card.value']+':</b> '+item.value+'</div>';
      }
      result += '<div class="kanban-item-stucked  small"><b>'+mauticLang['plugin.powerticpipes.stucked']+':</b> '+item.stucked+'</div>';
